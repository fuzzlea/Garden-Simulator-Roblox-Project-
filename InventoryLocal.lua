...
local mainGui = gui.Parent.Parent:WaitForChild("MainUI")
...
local plrGamepasses = plr:WaitForChild("Gamepasses")

local gamepassNoCollisions = plrGamepasses:WaitForChild("NoCollisions")
...
local plot = plr:WaitForChild("Plot").Value
...
local function changeGridSize()
	...
	if gridTexture ~= nil then
		...
		gridTexture.StudsPerTileU = GridSizes[table.find(GridSizes,gridSize)]
		gridTexture.StudsPerTileV = GridSizes[table.find(GridSizes,gridSize)]
...
local function deleteItem(cancel)
	...
	plrMouse.TargetFilter = plot:WaitForChild("Sounds")
	...
	DeleteConnections.MouseMove = plrMouse.Move:Connect(function()
		...
		if plrMouse.Target.Parent and plrMouse.Target.Parent:IsA("Model") and plrMouse.Target.Parent.PrimaryPart and plrMouse.Target.Parent.PrimaryPart.Name == "Base" then

			selectedPart = plrMouse.Target.Parent

			if selectedPart.Parent.Parent ~= plot then return end
	...
	DeleteConnections.ButtonUp = plrMouse.Button1Up:Connect(function()
		...
		if selectedPart then
			...
			deleteRemote:FireServer(selectedPart)
			selectedPart = nil
			sounds["Delete"]:Play()
...
local function clickItem(item, parent)
	...
	ghostItem:SetPrimaryPartCFrame(CFrame.new( math.floor(plrMouse.Hit.Position.X / gridSize + 0.5) * gridSize , plrMouse.Hit.Position.Y + ghostItem.PrimaryPart.Size.Y / 2 , math.floor(plrMouse.Hit.Position.Z / gridSize + 0.5) * gridSize ) * CFrame.Angles( 0 , math.rad(rotateAmount) , 0))
	...
	local function updateBuildPlaceable()

		if not ghostItem then return end
		
		if not ghostItem.PrimaryPart then return end
		
		if not ghostItem.PrimaryPart:IsA("BasePart") then return end
		...
		for _, part in workspace:GetPartsInPart(ghostItem.PrimaryPart) do
			...
			if part.Name == "Base" then
				...
				if gamepassNoCollisions.Value == false then
					...
					if table.find(AlwaysCollide, parent) or table.find(AlwaysCollide, Items:FindFirstChild(part.Parent.Name, true).Parent.Name) then
	...
	local function position()
		...
		newTween.Completed:Connect(updateBuildPlaceable)
...
local function updateIndex()
	...
	for _, item in path:GetChildren() do
		...
		model3d:SetCFrame(CFrame.Angles(0,90,0))
		...
		if itemInInv.Value > 0 then
			...
			model3d.AdornFrame.Ambient = Color3.fromHex("#000000")
			model3d.AdornFrame.LightColor = Color3.fromHex("#000000")
...
local function changeMenu(menu)
	...
	local function hideEverything()
		...
		-- show on hide
		if gridTexture ~= nil then gridTexture.Transparency = 1 end
		mainGui.Enabled = true
	...
	if menu == "Build" then
		...
		if gridTexture ~= nil then gridTexture.Transparency = 0.3 end
...
plr:WaitForChild("Plot"):GetPropertyChangedSignal("Value"):Connect(function() 

	plot = plr:WaitForChild("Plot").Value 
	gridTexture = plot:WaitForChild("GridTexture")
